...
namespace="drastha-tls-pos"
host_link="d-tpos-host"
client_link="d-tpos-client"
if [[ -e "$raw" ]] || ip netns list | grep -q "^$namespace " || ip link show "$host_link" >/dev/null 2>&1; then
  printf 'Refusing to overwrite/reuse positive TLS lab evidence\n' >&2
  exit 1
fi
mkdir -p "$raw/zeek"
cleanup() {
...
ip address add 10.39.0.1/30 dev "$host_link"
ip link set "$host_link" up
ip netns exec "$namespace" ip address add 10.39.0.2/30 dev "$client_link"
ip netns exec "$namespace" ip link set "$client_link" up
ip netns exec "$namespace" ip link set lo up
if [[ -n "$(ip netns exec "$namespace" ip route show default)" ]]; then
  printf 'Private lab unexpectedly has a default route\n' >&2
  exit 1
fi
...
tcpdump_pid=$!
sleep 1
python3 "$root/scripts/lab_tls_exchange_v2.py" server --cert "$raw/lab.crt" --key "$raw/lab.key" \
  >"$raw/server.stdout.txt" 2>"$raw/server.stderr.txt" &
server_pid=$!
sleep 1
ip netns exec "$namespace" python3 "$root/scripts/lab_tls_exchange_v2.py" client \
...
printf 'Generated fresh isolated TLS positive-control capture in %s\n' "$raw"
